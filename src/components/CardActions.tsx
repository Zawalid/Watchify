    ...
    <>
      <Dropdown classNames={{ content: 'blur-bg  backdrop-blur-2xl' }} backdrop='opaque' radius='sm'>
        ...
        >
          <DropdownItem
            key='watchlist'
            startContent={icons.watchlist}
            endContent={exists ? icons.added : null}
            onPress={async () => {
              if (exists) return;
              const id = toast.loading('Adding to watchlist...');
              await addWatchlistItem(media);
              setExists(true);
              toast.success(`"${(media as Movie).title || (media as TvShow).name}" was added to your watchlist`, { id });
            }}
          >
          ...
          <DropdownItem key='favorite' startContent={icons.favorite}>
            Favorite
          </DropdownItem>
        </DropdownMenu>
      </Dropdown>
